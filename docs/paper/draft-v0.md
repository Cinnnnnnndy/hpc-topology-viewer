...
> ⚠️ **仍是全文最缺的一章，但现在有了明确结论：能用什么、不能用什么，都已查清。**
> 完整分析见 `docs/paper/evidence-graft.md`（真实 profiling 报告的嫁接效度判断）。

### 8.0 结论先说：真实 profiling 报告不能当 evaluation

我们评估了能否把 `compute-graph-viewer` 仓库的 8 份真实 Ascend 910B profiling 报告
（涵盖 verl RL 训练、Megatron 16 卡、eager 推理等场景）嫁接为本文的验证素材。**结论：**

| 用途 | 成立性 |
|---|---|
| Introduction 的标本（证明 P4 描述的错误在真实场景里有真代价） | **部分成立**——只支撑 P4 一条 |
| System 的示例数据（喂真实数字让图不是玩具） | **部分成立且比预期窄**——真正落盘可用的原始数据只有 3 个文件 |
| **Evaluation**（证明框架本身有效） | **不成立** |

不成立的四条理由（任一条单独就够拒稿，完整论证见 `evidence-graft.md` §2）：
1. **归属错位**——这些结论是另一套工具（msprof-analyze + MindStudio Insight）产出的，
   没有一条是用本框架的视图看出来的
2. **覆盖率近乎为零**——A1–A8 里七条测配置校验与视觉编码，与 profiling 数据正交，只有 A5 沾边
3. **北极星指标（误判率）零样本**——它按定义只能来自人类被试，不可能从机器生成的报告析出
4. **一份报告（pangu2.0flash）被查出数据完整性问题**——落盘证据是另一数据集的字节级复制，
   部分数字系"构造示意"而非实测，**在原始数据回源核对前禁止引用**，见 §8.0.1

**能用的部分**：level2 数据集（唯一有公开来源链接、合规最干净）可以支撑 P4 的标本举例；
verl 报告里工程师"证据不足、拒绝下结论"的一手记录，是 DP-5「宁缺毋误」的绝佳佐证。
P1/P2/P3 三类错误在这批数据里**零实例**（CP 全等于 1），不能拿这批数据论证它们，
要另找真实的 Ulysses CP/SP 双语义作业，或走 §8.2 的受控实验。

#### 8.0.1 ⚠️ 一份数据不可用，需知会协作方

`pangu2.0flash_profiling_analysis_20260715` 报告声称的 32 卡 Pangu 数据，其 evidence 文件
经 md5 核对**与另一份报告（8 卡 Qwen2）的文件逐字节相同**，正文数字与落盘 CSV 对不上，
且一处关键分布被文件自身注明是"构造示意"。这是 `compute-graph-viewer` 那份报告自身的
数据完整性问题，不是本仓的责任，但既然发现了就不能装作没看见——建议告知该报告的作者核查，
论文侧则该数据整份排除。
...
真实数据既然不能当 evaluation，误判率实验就是必做项，不是备选项。完整设计见
`docs/paper/evidence-graft.md` §4，四种站得住的设计（可组合）：

1. **回溯性 incident 复现**（8–12 周）——拿真实 incident 当任务，24–36 人被试比正确率与耗时，
   直接产出 §11 北极星指标。三个任务全落在 P4 一类，**必须**配合设计 4 补 P1/P2/P3
2. **视图充分性审计**（2–3 周，无被试，最快出结果）——把 45 个真实问题点拆成最小信息元组，
   判定本框架哪个视图能读出、哪些是 miss。产出的是 expressiveness/coverage 类证据
3. **专家逐条验收 + 现有工具存在性举证**（3–4 周）——先用现有工具的公开截图证明 P1–P4 真实存在
   （不靠 profiling 数据），再请专家按 A1–A8 打分
4. **预注册的误判诱发实验**（4–6 周）——**专补 P1/P2/P3**，不依赖任何真实 profiling 数据，
   不受合规审批阻塞，是最该优先跑的一项

**建议顺序**：先跑 2（最快出结果、无需被试）与 4（专补数据缺口、不受合规阻塞），
再视资源决定要不要补 1（工期最长但打的是北极星指标本身）。

---

## 9. Limitations

### 9.0 ⚠️ 自查发现：实现在 A2 上违反了自己的核心主张

必须在这里正面写，不能藏在附录：本仓当前实现的开屏说明文字（"每个方框代表一个 Rank"）
与 3D 视图的选中高亮几何体（注释自称"选中的是一个 rank"）**都把 rank 画成了几何体**，
直接违反 P3/DP-2（"地址只标不画"）——这正是论文最核心的主张。完整核实记录见
`docs/paper/self-audit-and-retrofit.md` §2。

两条路：① 论文投稿前把实现改对（估计工作量：加一层"卡≠rank"的数据建模 + 改两处文案，
非大工程）；② 改不完就在此处坦白写出这个矛盾，并说明"当前版本在此处是已知的实现债务，
不影响本体论与信息架构层面的论证"。**选①更好**——一篇宣称"地址不是形状"的论文，
被审稿人在自己的 demo 里找到反例，是可以避免的低级失误，不该赌审稿人不会点开链接。

另有一条独立发现：论文原计划宣称的"工具内置 A1–A8 验收自检面板"**在当前发布产物里
用户根本打不开**（`ui.drawer==='acc'` 无任何触发路径），且面板里 A1/A2 两行是硬编码字符串
而非真断言。这条更容易改（补一个入口按钮 + 把 A1/A2 换成真断言），投稿前必须修，
否则"可判定验收"这条卖点会被现场演示戳穿。
...
| 「零验证是最硬的伤」 | **仍然成立，且现在有了更精确的诊断**：真实 profiling 报告不能顶替误判率实验，见 §8.0 与 `docs/paper/evidence-graft.md` |
| 「P-A 已完成 6/9 章」 | **修正为 5/10 且带一处新增负分**：Related Work 有骨架未精读、Evaluation 与 System 仍空、**新发现 A2 判据被自己的实现违反**（§9.0），这条在投稿前必须先修代码 |
| （新增）真实 profiling 数据能否顶替评测 | **不能。** 完整判断见 `docs/paper/evidence-graft.md`；另发现一份数据（pangu2.0flash）有数据完整性问题，需知会协作方 |
| （新增）场馆现金成本 | 见 `docs/paper/venue-costs.md`——多数顶会/期刊订阅制不收发表费，真正硬成本是作者注册费；ISC 2027 Poster 免费参会证是当前性价比最高的窗口 |
