...
- **上**：这一跑整体健不健康。大盘原先钉在右下角，那个位置不答任何方向上的问题——
  它就只是「还有一格」。告警挪成悬浮之后这一带空了出来，大盘补上来，四个方位才齐：
  抬头看整体，往左看结构，往右看某一件事怎么传的，往下看一个 step 里的时序。
...
视口窄到 1386 以下时，居中的条会和右上角那排筹码碰上（条宽约 466、筹码宽约 446 且右贴边，
`(V+466)/2 > V-460` 解出来的），取 1400 作断点，这时它整体下移一档让到筹码底下，和摊开的
抽屉走同一个避让规则。
...
尺寸 **920 × 144 是量出来的下限**，不是估的：再窄一档（880）「Router FP8 溢出，E193 吸收
98% token」那条标题就被 `line-clamp` 截掉；再矮一档（138）会一次截掉 5 条。原先是
1180 × 238——卡片在行里是 `stretch` 撑满的，多出来的高度全变成卡片下半截的空白，看着像
留白，其实只是没人要的那截。
