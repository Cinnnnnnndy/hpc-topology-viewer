...
**那排筹码代表的六格一进来就是摊开的**——它们是这一页的常备面：故事线给时间线，
整网 / 单维切分 / 卡内解剖给结构的三个粒度，泳道给时序，大盘给读数。不该让人先去点
一遍才看得见。事件相关的那几格（传播 / 机制 / 证据 / 血缘 / 单卡）仍按需展开，因为它们得先有
一个选中的事件才有内容。

把六格全关掉时 URL 写的是 `open=none` 而不是把参数删掉：删掉的话下次打开这条链接会被
当成「没指定」，六格默认又全长回来——读者刚一个个关掉、刷新一下全回来，那是 bug 不是
默认值。
