...
默认是**收起的一条告警**，钉在画布**正上方居中**：`● 6` ＋ 问题1 / 问题2 两枚段控。
点红点那一枚、或点右上角那枚「告警」筹码才铺成事件链。默认不铺开是因为铺开要占掉三分之一
...
视口窄到 1360 以下时，居中的条会和右上角那排筹码碰上（条宽约 428、筹码宽约 446 且右贴边，
`(V+428)/2 > V-460` 解出来的阈值是 1348），这时它整体下移一档让到筹码底下，和摊开的抽屉
走同一个避让规则。

摊开时它是不透明的，所以镜头落位会把它盖住的那一带让出来（按元素实测位置算，不写死，
窄屏时它自己会下移一档）。一开一关都重新落位一次，否则让出的带子和镜头对不上。
