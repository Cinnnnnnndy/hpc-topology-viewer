...
- **左上角是视图标题块**（对齐 sidecar 的标题写法）：kicker `PARALLEL LAYOUT` +
  名称「逻辑魔方 · 并行域重排」+ 一行当前规格（`TP8 · PP5 · DP100 · EP2 = 4000 rank ·
  50 个 A2A 域`，随配置更新）；
- **工具栏只常驻「形态 / 视角」**（两者同一行，中间一根细分隔；「更多」是独立按钮）——它们决定画面本身怎么摆；着色 / 注入 / 连线 / 时间 /
