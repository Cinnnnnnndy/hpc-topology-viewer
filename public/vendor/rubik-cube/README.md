...
4. **2D 可读性约束**：3D 摆法必须同时对每个正交视角成立。任一视角下屏幕两根轴的
   **最细步距之比 ≤ 4**——否则一根轴被拉成稀疏条纹（例：DP 平铺的板在顶视里只有
   ...
   块间距因此以「同屏另一轴最细步距 × 4」封顶（`clampFor2D`）。
5. **网格线 = 结构边界**：分块形态的背景网格按「块的格边界」推出（板/墙中心 ± 半格），
   不再按 rank 包围盒——后者会整体偏掉半个板宽，网格与内容对不上；EP 的域向网格线
   直接落在真实域位置上（域少逐域画、域多等间隔抽样），不再是与内容无关的等分线。
6. **视角收编 + 折叠如实标注**：若某视角把「块维」和「块内维」同时折进视线、剩下的
   信息塌陷，则不给出该视角（DP 平铺的侧视：4000 卡只剩 50 个孤立点 → 已略去，视角行
   给出原因）。保留的视角一律用粒度贴士如实报出**每格重叠卡数 = 各折叠维成员数之积**
...
- **轴测 3D + 正交 顶/前/侧 2D** + 被折叠深度维的**剖面逐层翻** +
  ...
  **拖动即从当前朝向无缝转回 3D 轴测**。视角按「每个 2D 平面只属于一个形态」
  ...
  强调读法，间距因此放开拉大；被略去的视角在视角行给出原因。）
- **时间轴**（左上配置卡片内）：播放/暂停 + 可拖游标定位任一时刻（60s 循环），
  驱动状态热力的时间维；
