...
**盘古ProMoE 8·5·100·2** —— 真实训练策略 TP8·EP2·PP5·4K NPU，出处
...
细看结构。回默认 1024 规格直接在输入框填 2·4·128·8 即可。
...
- **轴测 3D + 正交 顶/前/侧 2D** + 被折叠深度维的**剖面逐层翻** +
  ...
  **拖动即从当前朝向无缝转回 3D 轴测**。视角按「每个 2D 平面只属于一个形态」
  收编：格阵三平面（DP×TP·TP×PP·DP×PP）由**标准**独占，DP平铺/EP聚簇 的
  三个独有平面全保留，**TP切片/PP流水 只保留轴测**（它们与标准共享坐标系、
  2D 投影完全重合；价值在 3D 的「墙拉开/段拉开」强调读法，间距因此放开拉大），
  形态行内有指路提示；
