...
- **标注按视角出**（`axOnly`）：正交 2D 把一根轴折进视线，标记在那根轴上的偏移随之失效
  ...
  - **牌必须贴着它标的那块，并牵一根引线回去**（`axPlaceBlockLabel` + `axLeader`）：
    一排牌摆在下面、离场边两三个牌高，读者根本对不上号——错行尤其致命，第二行看上去
    像另一组东西。落位靠**量出来的牌高**（`sprite.scale.y`）而不是猜的常数：牌高随文案与
    `maxWorld` 变，写死的偏移要么叠在一起要么飘出去。row0 贴场边、row1 紧接其下（留 6%
    呼吸），引线消除「哪张牌对哪块」的歧义。
