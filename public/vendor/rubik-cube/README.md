...
- **连线端点有回归脚本兜底**（`scripts/verify-rubik-cube.mjs`）：两套配置 × 五形态逐条
  断言「每条通信线的端点落在两端 rank 的卡心」。「线没连到卡上」出现过两次，成因每次不同
  ——① 走线用样条会外扩成弧；② 飞行动画按帧固定系数缓动，4000 卡时帧率掉到 10fps 上下，
  `cur` 迟迟到不了 `target`，卡与线一起停在半路。现在动画改成**按时间**缓动（0.5s 走完，
  与帧率无关）并在收尾**精确吸附** `cur = target`；`handle.scene` 作为只读挂点供校验；
