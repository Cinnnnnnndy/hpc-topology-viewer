...
**视觉：黑白极简数据界面**（本页的视觉提示词）：
- 只用中性灰，所有颜色 R＝G＝B。背景 `#111111`，表面 `#131313–#282828`，主文字 `#E8E8E8`，次文字 `#A0A0A0`；
  纯白 `#FFFFFF` 只给当前选中对象的边框与关键高亮。嵌进来的矩阵、整网图、泳道、魔方由宿主套 `grayscale(1)`，
  不改它们的源码。
- **灰度就是数据**：rank 格的灰度 = 占用率（合计 / HBM），全页同一把尺，四档用矩阵本体的两条警戒线：
  `<70%` `#2E2E2E`、`70–88%` `#5C5C5C`、`88–100%` `#9A9A9A`、`>100%` `#E8E8E8`。越满越亮。物理图、板视图、
...
- **PP 段柱状选择器**（左列）：每段一根柱、共用基线，柱高 = 段内占用率最高那张卡，灰度同画布，虚线是 100%；
  只有选中那根柱顶上写数值，点柱进段。
- 左列不套卡片：标题 24px、规模两行、柱状选择器、图例；右列是 rank 读数；故障问题是一行文字 + 分隔线，
  严重度只分明暗。Inter 等无衬线，字重 400–500，数字等宽。
