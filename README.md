...
| `/parallel-topology/demo.html` | **并行拓扑可视化工具 · Demo v0.2（应用形态）**：首屏视图**一个 step · 五幕**——跟住一张卡（默认 g=41 @ tp2·cp1·pp4·dp16 / 48 层 / H=8192）走完一个 step，把**只做一次**（0 开机：128 个一模一样的进程；1 报到：一维编号→四位门牌，三个并行组是**算出来的**不是配出来的；2 备料：只实例化 L12–L23，**整网从头到尾只存在于代码里**）与**每个 micro-batch 重来**（3 进料 / 4 开算 / 5 接力与收束）分成两类；第 2 幕放大的那一层是 **openPangu 真图**（用图自带的预排布局画真节点·真层级·真连边，切分标注取 `shardOfNode` 的真结果，含 SP）；第 4 幕是**两条链唯一的交点**——算子与 kernel 队列在 kernel 处碰头。其余：主视图**单维切分**——一个切分一个视图，模板取参照系最后一张总图：粒度做竖轴（输入→整网→层→算子→kernel），rank 柱=容器贯穿全高，**块的横跨=共有、归属边界线（§02）随维移动、索引五元组条（§03）常驻**；切分的最小单位是片/段/桶而不是卡，卡只是各维区间交集的落位容器。另有合成·卡阵（五维相乘 + 通信 + 训练步动画）、卡内解剖（容器打开：层→算子→kernel→流）、**映射**（两个视角：**双链映射**——逻辑侧 5 级与物理侧 5 级都是包含链却**级与级不对齐**，所以连线是斜的；四条并行映射的源头在逻辑侧四个不同层级，**落点全是「卡」**，而 `world_size=TP×CP×PP×DP` 是计数恒等式不是刀、落在集群；上两级只决定拓扑与带宽、下两级（算子融合 / tiling）与并行完全无关。把「卡」画成整个平面（机内 8 卡 × N 台机，全量），**TP 必须机内 / PP 可跨机**当场可验，TP=16 会被标红。**交集腔**——两只索引空间盒：权重体 ℓ×h×e 三轴 / 激活体 ℓ×h×b×s 四轴；每把刀是一整片板，三片板相交剁出这张卡的那一腔，体积∶全盒 = 持有占比。DP 在权重体上没有轴 —— 腔的三条边长里没有 d ⇒ 体积不变 ⇒ **A3「加 DP 不买余量」的几何证明**；EP 在激活体上没有轴，路由是动作不是切分）、时间·流（1F1B + AllReduce 归因）。默认 128 卡（tp2·cp1·pp4·dp16·ep8，EP 折入 DP）。URL 即状态（`?view= &cut= &sel= &dim= &prim= &preset= &theme= &act= &p= &knife= &stack= &yaw= &pitch=`，旧 `v1..v5` 别名兼容），明暗主题随 token 翻转，A1–A8 页内实时自检 |
