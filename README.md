...
| 格子 | 内容 | 我们给了什么 |
|---|---|---|
| 舞台左 · 整网图 | iframe → `/patterns/net-slicing/pattern.html?cuts=pcte&rank=0&vtab=side` —— **同一个并行拓扑 pattern 的第二份配置** | 格头：名字 + 收起（藏掉 pattern 自己那枚名字，两格同源都叫「并行拓扑」会分不出谁是谁） |
...
| 默认机位 | `vtab=side`（侧视） | 3D |

左格问「一张整网被切成什么」——DP 是「整套安排复印几份」，复印件对这一问没有信息量，
落下来只会把同一张图重复 16 遍，所以那一刀不落；右格问「切完之后落成 world 张卡」。
默认落侧视，是因为三个机位里**只有它给出逐层的点击入口**（48 个
`[data-act=qval][data-k=layer]` 命中区，实测 3D / 正视都是 0 个），联动要从这一格
发起就得靠它；3D / 正视 / 顶视 一个没拿掉，pattern 自己那排段控点一下就换。
