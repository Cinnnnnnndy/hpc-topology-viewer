...
把 compute-graph-viewer 的 **config-relation-observer**（配置关系观测台）与本仓的
**组合工作台**合成一页，第三份输入是 `pangu_sophon_pytorch` 的训推指标口径：

| 出处 | 拿走的是 |
|---|---|
| `config-relation-observer` | **故事线与数据**：两条问题线、11 个运行事件、传播源→受影响、5 套机制相位、11 张证据图与全部读数。数值逐条照搬没改 |
| `/combo-workbench/` | **形制与视觉语言**：PTO token、ide-frame 的渐变光晕底与磨砂面板、顶栏 + 舞台骨架、URL 即这一屏、`pto:state` 主题桥 |
| `pangu_sophon_pytorch` | **训推指标口径**：十条「关键少数」各由哪段代码算出、期望往哪个方向走，加五个早停钩子。只取口径不取值 |

新出的那一件事是**编排**。组合工作台是「摞格子」；这一页是「一个主视图 + 四周卫星」，
而且**「无限画布」是字面意思——下钻不把中心那块换掉，是把整个星系搬到画布上的一个新原点**，
原地留一张虚线轨迹卡，两站之间用长曲线连起来。画布因此随探索变大：缩小就看见走过的整条路径，
点任一张轨迹卡原路退回。一路走下去六站：
整网 2048 卡 → 事件传播图 → 机制相位（四相可播）→ 单卡解剖 → 时间泳道 → 训推大盘。

整页只有一条下钻链（`levelChain()`）：主视图看哪一档、面包屑有几级、画布上走到第几站，
三件事全从它读——以前这类判断各写一份，加一档就得三处同时改。

五种机制（路由塌缩 / barrier 语义 / 依赖链回压 / 存活区间叠加 / 碎片）各有一套自绘 SVG，
不共用「通用示意图」：五条机制本来就不是同一件事。比如依赖链回压那套把「2048 卡停摆」
拆成一条可验算的乘法链 `1 →×EP64→ 64 →×EDP8→ 512 →×PP4→ 2048`，每一步都是一个已知的
并行度，而不是一句「故障扩散」。

大盘那一档**只取口径不取值**：每个数都能在故事线的同一个事件里找到原文，没采到的格子写
「—」，不替它编——大盘上最容易骗人的正是「看起来每格都有数」。

整页自包含单文件（token、样式、数据、图元全在一份里，零外部依赖、不引 CDN），
所以它也可以直接当成组合工作台上的一格嵌进去（认 `?embed=1` 收顶栏、认 `pto:state`）。
入口页：`public/incident-canvas/index.html`；发布后两个入口 `/incident-canvas/`（短链接）
与 `/patterns/incident-canvas/pattern.html`（规范位置，`pattern.json` 契约同级）。
细节见 [`public/incident-canvas/README.md`](public/incident-canvas/README.md)。
